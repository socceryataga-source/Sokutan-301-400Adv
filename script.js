(()=>{
  ...
  let quizItems=[],currentIndex=0,score=0,answered=false,lastConfig={start:1,end:100,mode:'ordered'};
  ...
  function clampRange(){let s=Number(startNo.value),e=Number(endNo.value);const minId=data.length?Math.min(...data.map(q=>q.id)):1,maxId=data.length?Math.max(...data.map(q=>q.id)):100;if(!Number.isFinite(s))s=minId;if(!Number.isFinite(e))e=maxId;s=Math.max(minId,Math.min(maxId,Math.floor(s)));e=Math.max(minId,Math.min(maxId,Math.floor(e)));if(s>e)[s,e]=[e,s];startNo.value=s;endNo.value=e;const count=data.filter(q=>q.id>=s&&q.id<=e).length;rangeInfo.textContent=`${s}〜${e}の${count}問を出題します。`;return{start:s,end:e,count}}
